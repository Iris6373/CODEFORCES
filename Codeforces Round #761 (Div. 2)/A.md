...
## My solution :  (Accepted)
    ...
    If T is not "abc", ans is sorted string of S.
    Else, we place all c's before b's inorder to avoid the subsequence "abc".
   ...
   ### Code : (.cpp)  
        ...
        void solve() {  
            ...
            n = sz(s);
            // de(n);
            if (t == "abc" && mp[0] > 0 && mp[1] > 0 && mp[2] > 0) {
                r = "";
                for (i=0;i<mp[0];i++) r += 'a';
                for (i=0;i<mp[2];i++) r += 'c';
                for (i=0;i<mp[1];i++) r += 'b';
                for (i=3;i<26;i++) {
                    for (j=0;j<mp[i];j++) r += ('a'+i);
                }
                s = r;
            }
            cout << s << "\n";
    ...
    Time Complexity  : O(t*n*logn)
                       t test cases
                       Since in each test case we sort the string.
