...
## My solution :  (Accepted)
   ...
   ### Code : (.cpp)  
          ...
          ll chk(vt v, ll n, ll mid, ll a) {
              ll i, ans = 0;
              for (i=1;i<n;i++) {
                  if (v[i-1]+mid < v[i]) ans += mid;
                  else ans += v[i]-v[i-1];
              }
              ans += mid;
              return ans >= a;
          ...
          void solve() {  
              ll i, j, k, m, n, a=0, b=0, c=0, d=0, e=0, f=0, w=0, x=0, y=0, z=0, ans = 1e18;
              string s;
              cin >> n >> a;
              vt v(n);
              f(n) {
    ...
    Time Complexity  : O(t*n*log(n))
                       t test cases
                       Each test case takes linarithmetic time.
